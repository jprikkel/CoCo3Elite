...
module uart_video_trace_tb;
 reg clk=0,reset=1;
 wire tx;
 reg [8*130-1:0] expected={"PC=1234 K=1 R=FF C=FF S=80 T=00 V=807E00D620980123456789ABCDEF0123456789AB G=4C204 M=38393A3B3C3D3E3F3031323334353637 D=12345678",8'h0d,8'h0a};
 reg [7:0] received;
 integer i,b;
 always #5 clk=~clk;
 ...
 .sdram_debug_status(32'h12345678),.uart_tx_o(tx));
 initial begin
  ...
  for(i=0;i<130;i=i+1) begin
   ...
   if(received!==expected[8*(129-i)+:8]) $fatal(1,"UART byte %0d got %h expected %h",i,received,expected[8*(129-i)+:8]);
  end
  $display("PASS: UART emits complete 130-byte PC/SD/video/MMU/SDRAM snapshot at 115200 baud");
  $finish;
 end
