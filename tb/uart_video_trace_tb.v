...
module uart_video_trace_tb;
 reg clk=0,reset=1;
 wire tx;
 reg [8*76-1:0] expected={"PC=1234 K=1 R=FF C=FF S=80 D=00 V=807E00D620980123456789ABCDEF0123456789AB",8'h0d,8'h0a};
 reg [7:0] received;
 integer i,b;
 always #5 clk=~clk;
 ...
 .sd_status(8'h80),.sd_detail(8'h00),
 ...
 initial begin
  ...
  for(i=0;i<76;i=i+1) begin
   ...
   if(received!==expected[8*(75-i)+:8]) $fatal(1,"UART byte %0d got %h expected %h",i,received,expected[8*(75-i)+:8]);
  end
  $display("PASS: UART emits complete 76-byte PC/SD/video snapshot at 115200 baud");
  $finish;
 end
