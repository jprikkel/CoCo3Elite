...
module sync_wait_tb;
 ...
 .joystick_left_x(6'd32),.joystick_left_y(6'd32),.joystick_left_fire(1'b0),.joystick_left_fire2(1'b0),
 .joystick_right_x(6'd32),.joystick_right_y(6'd32),.joystick_right_fire(1'b0),.joystick_right_fire2(1'b0),
 .sd_status(8'd0),.sd_detail(8'd0),.video_hsync(1'b1),.pia_hsync(1'b1),.video_vsync(vsync),.video_address(20'd0));
 always @(posedge clock)
  if (dut.io_write && dut.address==16'hff70) test_status <= dut.write_data;
 initial begin
  ...
  repeat(10) @(negedge clock); vsync=0;
  repeat(10) @(negedge clock); vsync=1;
  wait(test_status==8'hfe || test_status==8'hee);
  if(test_status==8'hee) $fatal(1,"FAIL: CPU read/ack stage %0d",stage);
