...
# Override part/mode: vivado -mode batch -source ... -tclargs <part> CPU_DIAGNOSTIC
...
set mode       [string toupper [expr {$argc > 1 ? [lindex $argv 1] : "HDMI_COCO_AUDIO"}]]
...
if {$mode ni {HDMI_LIBRARY_TEST HDMI_COCO_AUDIO CPU_DIAGNOSTIC}} {
    error "Unknown mode '$mode'; use HDMI_LIBRARY_TEST, HDMI_COCO_AUDIO, or CPU_DIAGNOSTIC"
}
...
if {$mode eq "HDMI_LIBRARY_TEST"} {
    set_property verilog_define {HDMI_LIBRARY_TEST HDMI_LIBRARY_AUDIO} [current_fileset]
}

if {$mode in {HDMI_COCO_AUDIO CPU_DIAGNOSTIC}} {
    lappend sources \
...
if {$mode eq "HDMI_COCO_AUDIO"} {
    ...
    set coco3_defines {NEW_SRAM HDMI_LIBRARY_TEST HDMI_LIBRARY_COCO HDMI_LIBRARY_AUDIO HDMI_RASTER_800X525}
...
if {$mode in {HDMI_LIBRARY_TEST HDMI_COCO_AUDIO}} {
...
set bit_name [expr {$mode eq "HDMI_COCO_AUDIO"
                    ? "wukong_hdmi_coco_audio.bit"
                    : ($mode eq "HDMI_LIBRARY_TEST"
                       ? "wukong_hdmi_library_test.bit"
                       : "wukong_cpu_diagnostic.bit")}]
