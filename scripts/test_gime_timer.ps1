...
$runDir = Join-Path $repoRoot 'build\sim\gime_timer'
New-Item -ItemType Directory -Force -Path $runDir | Out-Null
Push-Location $runDir
try {
    & (Join-Path $VivadoBin 'xvlog.bat') `
        (Join-Path $repoRoot 'rtl\core\coco3_gime_timer.v') `
        (Join-Path $repoRoot 'tb\gime_timer_tb.v')
    if ($LASTEXITCODE -ne 0) { throw "xvlog failed: $LASTEXITCODE" }
    & (Join-Path $VivadoBin 'xelab.bat') gime_timer_tb -s gime_timer_tb_sim
    if ($LASTEXITCODE -ne 0) { throw "xelab failed: $LASTEXITCODE" }
    & (Join-Path $VivadoBin 'xsim.bat') gime_timer_tb_sim -runall
