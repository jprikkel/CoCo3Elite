...
set work [file join $repo build wukong-manager-smoke]
...
set sources [concat [lsort [glob [file join $core *.v]]] \
                    ...
                    [list [file join $repo rtl management ultraembedded_manager_smoke.v] \
                          [file join $repo rtl management wukong_manager_smoke_top.v] \
                          [file join $repo rtl wukong uart_tx.v]]]
set_property include_dirs [list $core] [current_fileset]
read_verilog $sources
read_xdc [file join $repo constraints wukong_manager_smoke.xdc]
synth_design -top wukong_manager_smoke_top -part xc7a100tfgg676-2
opt_design
place_design
phys_opt_design
route_design
report_timing_summary -delay_type min_max -report_unconstrained -max_paths 20 \
    -file [file join $work timing_summary.rpt]
...
if {[llength $worst_path] == 0} { error "No timed paths found; bitstream not generated" }
if {[get_property SLACK $worst_path] < 0} { error "Timing failed; bitstream not generated" }
write_bitstream -force [file join $work wukong_manager_smoke.bit]
puts "Wrote [file join $work wukong_manager_smoke.bit]"
