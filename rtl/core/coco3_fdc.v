...
// controller. It implements the register behavior used by Disk BASIC for
// restore, seek, force interrupt, and single-sector reads. Writes terminate
// with write-protect status.
module coco3_fdc (
    input  wire        clock,
    input  wire        reset,
    input  wire        io_read,
    ...
    coco3_disk_image #(.IMAGE_FILE("build/disks/intruders.mem")) drive0_image_i (
        .clock(clock), .address(image_address), .data(drive0_data)
    );

    coco3_disk_image #(.IMAGE_FILE("build/disks/daggorat.mem")) drive1_image_i (
                        ...
                        case (write_data[7:4])
                            ...
                            4'h8, 4'h9: begin // read single sector
                                ...
                                if (!drive_selected) begin
                                    status <= 8'h80; // not ready
                                    read_active <= 1'b0;
                                    nmi_pending <= 1'b1;
                                end else if (!valid_position) begin
                                    status <= 8'h10; // record not found
                                    read_active <= 1'b0;
                                    nmi_pending <= 1'b1;
                                end else begin
                                    status <= 8'h03; // busy and DRQ
                                    read_active <= 1'b1;
                                end
                            end
