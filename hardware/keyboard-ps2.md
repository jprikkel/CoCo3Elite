...
CoCo3Elite uses a PS/2 keyboard as its primary keyboard input. The
existing RTL receives PS/2 scan codes and converts them into the CoCo keyboard
matrix in the current Wukong builds.

This interface is implemented in `COCO3_BOOT`, `HDMI_COCO_TEST`, and
`HDMI_COCO_AUDIO`. Recorded hardware tests verify BASIC typing/editing,
`RUN`, `LIST`, and Break.
...
*Historical PS/2 test photo; the authoritative current signal assignment is
J14 pin 1/P23 data and pin 3/T24 clock. Check
...
The current `constraints/wukong.xdc` assigns the existing top-level ports:
...
is synchronized and decoded by the existing `rtl/PS2_Key/ps2_keyboard.v` logic,
...
- `rtl/PS2_Key/ps2_keyboard.v` — receives serial PS/2 frames and scan codes.
